    ...
    <div className="m-4 pb-10">
      ...
      <div className="my-2">
        ...
        <div className="flex flex-wrap gap-8">
          <Katex tex={`v_{0}=${constants.v0}\\frac{m}{s}`} />
          <Katex tex={`x_{m}=${constants.xM}m`} />
          <Katex tex={`h=${constants.h}m`} />
          <Katex tex={`g=${g}\\frac{m}{s^2}`} />
          <Katex
            tex={`\\alpha\\approx${((alpha * 180) / Math.PI).toFixed(
              3,
            )}^\\circ`}
          />
          <Katex tex={`\\text{Tiempo}=${t.toFixed(2)}\\text{s}`} />
          <Katex
            tex={`\\text{Velocidad actual del dardo}=${currentDSpeed.toFixed(2)}\\frac{m}{s}`}
          />
          <Katex
            tex={`\\text{Velocidad actual del mono}=${currentMSpeed.toFixed(2)}\\frac{m}{s}`}
          />
        </div>
      </div>
